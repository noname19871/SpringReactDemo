...
class App extends React.Component {
	...
	constructor(props) {
		super(props);
		this.state = {customers: [], attributes: [], page: 1, pageSize: 2, links: {}
	...
	loadFromServer(pageSize) {
		follow(client, root, [
			...
			this.page = customerCollection.entity.page;
		...
		}).then(customerPromises => {
			return when.all(customerPromises);
		}).done(customers => {
			this.setState({
				page: this.page,
				customers: customers,
	...
	onCreate(newCustomer) {
		follow(client, root, ['customers']).done(response => {
			client({
				method: 'POST',
	...
	onUpdate(customer, updatedCustomer) {
		if(customer.entity.manager.name === this.state.loggedInManager) {
			updatedCustomer["manager"] = customer.entity.manager;
			client({
				method: 'PUT',
				path: customer.entity._links.self.href,
				...
				if (response.status.code === 403) {
					alert('ACCESS DENIED: You are not authorized to update ' +
						customer.entity._links.self.href);
				}
				if (response.status.code === 412) {
					alert('DENIED: Unable to update ' + customer.entity._links.self.href +
						'. Your copy is stale.');
				}
			});
		} else {
	...
	// tag::on-delete[]
	onDelete(customer) {
		client({method: 'DELETE', path: customer.entity._links.self.href}
	...
	componentDidMount() {
		...
		stompClient.register([
			{route: '/topic/newCustomer', callback: this.refreshAndGoToLastPage},
			{route: '/topic/updateCustomer', callback: this.refreshCurrentPage},
	...
	render() {
		return (
			<div>
				...
				<CustomerList page={this.state.page}
...
class UpdateDialog extends React.Component {
	...
	render() {
		...
		const isManagerCorrect = this.props.customer.entity.manager.name == this.props.loggedInManager;

		if (isManagerCorrect === false) {
			return (
					<div>
						<a>Not Your Customer</a>
					</div>
...
class CustomerList extends React.Component {
	...
	render() {
		const pageInfo = this.props.page.hasOwnProperty("number") ?
			<h3>customers - Page {this.props.page.number + 1} of {this.props.page.totalPages}</h3> : null;

		const customers = this.props.customers.map(customer =>
			<Customer key={customer.entity._links.self.href}
					  customer={customer}
...
class Customer extends React.Component {
	...
	render() {
		return (
			<tr>
				<td>{this.props.customer.entity.firstName}</td>
				<td>{this.props.customer.entity.lastName}</td>
				<td>{this.props.customer.entity.manager.name}</td>
				<td>
