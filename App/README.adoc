...
== Linking customers with their managers

The last bit of modeling security is to associate customers with a manager. In this domain, an customer can have one manager while a manager can have multiple customers:
...
== Securing customers to their managers

Spring Security supports a multitude of options when it comes to defining security policies. In this section, you want to restrict things such that ONLY managers can view customer payroll data, and that saving, updating, and deleting operations are confined to the customer's manager. In other words, any manager can log in and view the data, but only a given customer's manager can make any changes.
...
On `save()`, either the customer's manager is null (initial creation of a new customer when no manager has been assigned), or the customer's manager's name matches the currently authenticated visitor's name. Here you are using http://docs.spring.io/spring-security/site/docs/current/reference/htmlsingle/#el-access[Spring Security's SpEL expressions] to define access. It comes with a handy "?." property navigator to handle null checks. It's also important to note using the `@Param(...)` on the arguments to link HTTP operations with the methods.

On `delete()`, the method either has access to the customer, or in the event it only has an id, then it must find the *customerRepository* in the application context, perform a `findOne(id)`, and then check the manager against the currently authenticated visitor.

== Writing a `UserDetails` service

A common point of integration with security is to define a `UserDetailsService`. This is the way to connect your visitor's data store into a Spring Security interface. Spring Security needs a way to look up users for security checks, and this is the bridge. Thankfully with Spring Data, the effort is quite minimal:
...
`SpringDataJpaUserDetailsService` implements Spring Security's `UserDetailsService`. The interface has one method: `loadUserByUsername()`. This method is meant to return a `UserDetails` object so Spring Security can interrogate the visitor's information.
...
* Logout is configured to take the visitor to "/".
...
A good visitor experience is when the application can automatically apply context. In this example, if a logged in manager creates a new customer record, it makes sense for that manager to own it. With Spring Data REST's event handlers, there is no need for the visitor to explicitly link it. It also ensures the visitor doesn't accidentally records to the wrong manager.
...
In this sitation, you lookup the current visitor's security context to get the visitor's name. Then look up the associated manager using `findByName()` and apply it to the manager. There is a little extra glue code to create a new manager if he or she doesn't exist in the system yet. But that is mostly to support initialization of the database. In a real production system, that code should be removed and instead depend on the DBAs or Security Ops team to properly maintain the visitor data store.

== Pre-loading manager data

Loading managers and linking customers to these managers is rather straight forward:
...
The one wrinkle is that Spring Security is active with access rules in full force when this loader runs. Thus to save customer data, you must use Spring Security's `setAuthentication()` API to authenticate this loader with the proper name and role. At the end, the security context is cleared out.
...
----
$ curl -v -u greg:turnquist localhost:8080/api/customers/1
*   Trying ::1...
* Connected to localhost (::1) port 8080 (#0)
* Server auth using Basic with visitor 'greg'
...
With all these mods in the backend, you can now shift to updating things in the frontend. First of all, show an customer's manager inside the `<Employee />` React component:

[source,javascript,indent=0]
----
include::src/main/js/app.js[tag=customer]
----

This merely adds a column for `this.props.customer.entity.manager.name`.
...
You can see the newly added manager column. Go through a couple pages until you find customers owned by *oliver*.
...
If you try *Delete*, it should fail with a similar message. Create a new customer, and it should be assigned to you.

== Review

In this section:

* You defined the model of manager and linked it to an customer via a 1-to-many relationship.
...
* You wrote another Spring Data REST event handler to trap creation events before they happen so they current visitor could be assigned as the customer's manager.
* You updated the UI to show an customer's manager and also display error pop-ups when unauthorized actions are taken.
...
Managers have access to customer data. Should customers have access? If you were to add more details like phone numbers and addresses, how would you model it? How would you grant customers access to the system so they could update those specific fields? Are there more hypermedia controls that would be handy to put on the page?
